...
def create_artwork(directory: Path, devkitpro: Path) -> tuple[Path, Path]:
...
def package(elf: Path, output: Path, devkitpro: Path) -> dict:
    ...
    makerom, dsxtool = find_tool("makerom", devkitpro), find_tool("3dsxtool", devkitpro)
    ...
    with tempfile.TemporaryDirectory(prefix="package-", dir=scratch) as temporary:
        ...
        icon, banner = create_artwork(stage / "artwork", devkitpro)
        romfs = stage / "romfs"
        romfs.mkdir()
        ...
        manifest = {"name": "2Ship 3DS experimental", "elf_sha256": sha256(elf), "validation": validation,
                    "artwork": "Original procedural typography and rectangles; silent banner audio",
...
def main() -> int:
    ...
    try:
        if args.artwork_only:
            create_artwork(args.output.resolve(), args.devkitpro.resolve())
        else:
            package(args.elf, args.output, args.devkitpro)
