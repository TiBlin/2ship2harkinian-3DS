...
from build_clock import prepare_source_clock, check_lus_ninja_manifest
...
def build_lus(devkitpro: Path, jobs: int = 4, configure_only: bool = False,
              *, clock_checked: bool = False) -> Path:
    if not 1 <= jobs <= 64:
    ...
    if not configure_only:
        check_lus_ninja_manifest(build, env)
