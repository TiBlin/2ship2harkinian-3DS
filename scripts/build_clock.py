...
def prepare_source_clock(root: Path, log: Callable[[str], None] = emit) -> dict:
    ...
    if not future:
        log("Horodatages des sources : OK (aucun fichier date dans le futur).")
        return report

    lead = max(info.st_mtime_ns - started_ns for _, info in future) / 1_000_000_000
    log(f"Horodatages futurs detectes : {len(future)} fichier(s), avance maximale {lead:.1f} s.")
    ...
    for cache in cache_paths:
        if cache.exists() or cache.is_symlink():
            if is_link(cache) or not cache.resolve().is_relative_to(root) or not cache.is_dir():
                raise RuntimeError(f"Cache externe ou non valide : {cache}. Aucun cache supprime.")
    for cache in cache_paths:
        if cache.exists():
            saved = cache.with_name(cache.name + ".avant-horodatages-" + suffix)
            if saved.exists():
                raise RuntimeError(f"Sauvegarde deja presente : {saved}")
            cache.rename(saved)
            report["cache_backups"].append({"from": cache.relative_to(root).as_posix(),
                                             "backup": saved.relative_to(root).as_posix()})
            log(f"Ancien cache conserve : {saved}")

    journal = root / "wizard-logs" / ("horodatages-" + suffix + ".json")
    ...
    for path, before in future:
        ...
        if is_link(path) or current.st_mtime_ns != before.st_mtime_ns or current.st_size != before.st_size:
            raise RuntimeError(f"Source changee pendant la verification : {path}. Relancer sans edition concurrente.")
        os.utime(path, ns=(before.st_atime_ns, target_ns))
        report["changed_count"] += 1
    report["completed"] = True
    journal.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    log(f"Horodatages corriges : {report['changed_count']} fichier(s); contenu inchange.")
    log(f"Rapport des horodatages : {journal}")
    return report
...
def check_lus_ninja_manifest(build: Path, env: dict[str, str],
    ...
    if not ninja:
        raise RuntimeError("Ninja introuvable pour verifier le manifeste libultraship.")
    ...
    try:
        ...
        raise RuntimeError("Verification Ninja trop longue; compilation arretee sans relance.") from error
    ...
    if result.returncode or dirty:
        ...
        log(output.rstrip())
        raise RuntimeError(
            "Manifeste Ninja libultraship encore perime juste apres CMake. "
            "Arret preventif : pas de boucle de 100 relances. "
            "Verifier l'horloge et le chemin d'entree indique par 'ninja explain' "
            "(les fichiers devkitPro externes ne sont jamais retouches). "
            f"Diagnostic : {diagnostic}")
    log("Manifeste Ninja libultraship : stable; lancement de la compilation.")
