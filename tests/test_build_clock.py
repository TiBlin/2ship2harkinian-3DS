...
@unittest.skipUnless(shutil.which("cmake") and shutil.which("ninja") and
                     any(shutil.which(name) for name in (os.environ.get("CC", "cc"), "gcc")),
...
class RealNinjaTests(unittest.TestCase):
    ...
    def run_command(self, cmd, timeout=30):
        return subprocess.run(list(map(str, cmd)), capture_output=True, text=True, timeout=timeout)
    ...
    def test_real_historical_100_regeneration_loop_then_fixed_compilation(self):
        ...
        # This control deliberately launches CMake 100 times, including on Windows.
        historical = self.run_command(["cmake", "--build", build], timeout=180)
    ...
    def test_real_gate_reports_external_future_input_without_mutating_it(self):
        ...
        self.assertIn(external.as_posix(), diagnostic.replace("\\", "/"))
