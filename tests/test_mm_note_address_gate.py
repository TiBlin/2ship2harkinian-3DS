...
Dependencies: Python 3 and g++ on Linux x86-64 (-no-pie for real low addresses).
...
import argparse, subprocess, tempfile, shutil, hashlib, json
...
def main():
    ...
    text=src.read_text(); body=function(text,'void AudioPlayback_ProcessNotes(void)')
    sync=function((args.root/'third_party/2ship/mm/src/audio/lib/synthesis.c').read_text(),'void AudioSynth_SyncSampleStates(s32 updateIndex)')
    ...
    with tempfile.TemporaryDirectory() as td:
        for name,code,defines,skip in variants:
            cpp=Path(td)/(name+'.cpp');exe=Path(td)/name
            cpp.write_text(f'#define EXPECT_SKIP {skip}\n#define LABEL "{name}"\n'+STUBS+code+sync+MAIN)
            subprocess.run([compiler,'-std=c++17','-O2','-fno-pie','-no-pie',*defines,str(cpp),'-o',str(exe)],check=True)
